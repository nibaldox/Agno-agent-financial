...
def plot_equity_curve(results: dict, filename: str = "equity_curve.png"):
    ...
    plt.axhline(
        y=results["config"]["initial_capital"], color="r", linestyle="--", label="Initial Capital"
    )
...
def print_detailed_report(results: dict):
    ...
    for i, trade in enumerate(results["trades"][:10], 1):  # Primeros 10
        ...
        print(
            f"{action_emoji} {trade['date']}: {trade['action']} {trade['shares']} {trade['ticker']} @ ${trade['price']:.2f}"
        )
...
def compare_results(filenames: list):
    ...
    for filename, results in results_list:
        metrics = results["final_metrics"]
        print(
            f"{filename:<30} | {metrics['total_return_pct']:>11.2f}% | {metrics['win_rate']:>9.1f}% | {metrics['profit_factor']:>15.2f}"
        )
