...
class AgentLoader:
        ...
        if "yfinance" in tools_config:
            ...
            if yf_config.get("enabled", True):
                if yf_config.get("include_all", False):
                    ...
                    tools.append(YFinanceTools(include_tools=yf_config["include_tools"]))
    ...
    def load_team(
        self, use_openrouter: bool = True, portfolio_summary: Optional[Dict] = None
    ) -> Team:
        """
        ...
        for stage_name, stage_desc in team_config["team"]["instructions"][
...
def load_risk_analysts(
    use_openrouter: bool = False, portfolio_summary: Optional[Dict] = None
) -> List[Agent]:
...
def load_trading_strategists(use_openrouter: bool = False) -> List[Agent]:
    ...
    return [
        loader.load_agent(
            "strategy_technical", "trading_strategists.yaml", "agents.technical", use_openrouter
        ),
        loader.load_agent(
            "strategy_fundamental", "trading_strategists.yaml", "agents.fundamental", use_openrouter
        ),
        loader.load_agent(
            "strategy_momentum", "trading_strategists.yaml", "agents.momentum", use_openrouter
        ),
    ]


def load_portfolio_manager(
    use_openrouter: bool = True, portfolio_summary: Optional[Dict] = None
) -> Agent:
    """Quick loader for Portfolio Manager agent"""
    loader = AgentLoader()
    return loader.load_agent(
        "portfolio_manager", use_openrouter=use_openrouter, portfolio_summary=portfolio_summary
    )


def load_daily_reporter(
    use_openrouter: bool = True, portfolio_summary: Optional[Dict] = None
) -> Agent:
    """Quick loader for Daily Reporter agent"""
    loader = AgentLoader()
    return loader.load_agent(
        "daily_reporter", use_openrouter=use_openrouter, portfolio_summary=portfolio_summary
    )
...
def load_complete_team(
    use_openrouter: bool = True, portfolio_summary: Optional[Dict] = None
) -> Team:
    """Quick loader for complete trading team"""
    loader = AgentLoader()
    return loader.load_team(use_openrouter=use_openrouter, portfolio_summary=portfolio_summary)
