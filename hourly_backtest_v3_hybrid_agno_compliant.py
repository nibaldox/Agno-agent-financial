...
class TradingDecision(BaseModel):
    ...
    reason: str = Field(
        max_length=300, description="Justificación técnica concisa (máximo 1-2 líneas)"
    )
    strategy: str = Field(
        default="",
        max_length=100,
    ...
    confidence: float = Field(
        ge=0, le=1, default=0.5, description="Nivel de confianza en la decisión (0-1)"
    )
    ...
    def calculate_technical_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "ema48_trend": (
                    "⬆️ ALCISTA" if close.iloc[-1] > ema48.iloc[-1] else "⬇️ BAJISTA"
                ),  # NUEVO
...
"""
        ...
        if len(close_hist) >= 2:
            price_change_1h = (
                (close_hist.iloc[-1] - close_hist.iloc[-2]) / close_hist.iloc[-2]
            ) * 100
        else:
            price_change_1h = 0
        ...
        if len(close_hist) >= 5:
            price_change_4h = (
                (close_hist.iloc[-1] - close_hist.iloc[-5]) / close_hist.iloc[-5]
            ) * 100
        else:
            price_change_4h = 0
        ...
        try:
                        ...
                        try:
                            ...
                            if "buy" in low:
                                ...
                                decision = TradingDecision(
                                    action="HOLD",
                                    amount=0,
                                    reason="Fallback: respuesta no parseable",
                                    strategy="heuristic",
                                    confidence=0.0,
                                )
            ...
            if decision is None:
                decision = TradingDecision(
                    action="HOLD",
                    amount=0,
                    reason="Fallback: respuesta no proporcionada o no parseable",
                    strategy="heuristic",
                    confidence=0.0,
                )
            ...
            if decision.action == "BUY" and current_price > 0:
                ...
                if amount == 0 or amount < 100:
                    ...
                    print(
                        f"[DEBUG] Monto auto-asignado: ${amount:.2f} ({pct*100:.1f}% del efectivo)"
                    )
...
def fetch_intraday_data(
    ticker: str = "BTC-USD", days: int = 7, interval: str = "1h"
) -> pd.DataFrame:
    """
...
def run_hybrid_backtest_v3(
    ...
    for i in range(len(df)):
        ...
        auto_sales = simulator.check_risk_limits(
