...
class VisualizationGenerator:
    ...
    def __init__(self, output_dir: str | Path = "reports"):
        ...
        if self.use_plotly:
            ...
            if not _HAS_MATPLOTLIB:
                raise ImportError("No visualization library available. Install plotly or matplotlib")
    ...
    def plot_performance_vs_benchmark(
        ...
        ax.plot(benchmark_norm.index, benchmark_norm.values,
                ...
                color=self.colors['secondary'], linewidth=2, linestyle='--', marker='s', markersize=4)
        ...
        ax.text(portfolio_norm.index[-1], final_portfolio + 1,
                f'+${portfolio_gain:.1f} ({portfolio_gain/starting_value*100:.1f}%)',
                color=self.colors['primary'], fontsize=10, fontweight='bold')
        
        ax.text(benchmark_norm.index[-1], final_benchmark + 1,
                f'+${benchmark_gain:.1f} ({benchmark_gain/starting_value*100:.1f}%)',
                color=self.colors['secondary'], fontsize=10)
    ...
    def plot_roi_bars(
        ...
        for idx, (ticker, roi) in enumerate(zip(plot_data['Ticker'], plot_data['ROI %'])):
            ax.text(roi + (2 if roi > 0 else -2), idx, f'{roi:+.1f}%',
    ...
    def plot_portfolio_composition(
        ...
        if holdings_df.empty:
            ...
            ax.text(0.5, 0.5, 'No holdings to display',
                   ha='center', va='center', fontsize=14)
            ax.axis('off')
        ...
        # Prepare data
        composition = holdings_df.groupby('ticker')['current_value'].sum().sort_values(ascending=False)
        ...
        legend_labels = [f'{ticker}: ${value:,.0f}' 
                        for ticker, value in composition.items()]
    ...
    def plot_drawdown_over_time(
        ...
        ax1.plot(equity_series.index, equity_series.values,
                label='Portfolio Equity', color=self.colors['primary'], linewidth=2)
        ax1.plot(equity_series.index, running_max.values,
                label='All-Time High', color=self.colors['success'], 
        ...
        ax2.fill_between(drawdown.index, drawdown.values, 0,
                        where=(drawdown < 0), color=self.colors['danger'], alpha=0.6)
    ...
    def plot_win_loss_analysis(
        ...
        ax1.bar(['Wins', 'Losses'], [wins, losses],
               color=[self.colors['success'], self.colors['danger']], alpha=0.7)
        ...
        ax3.hist([win_pnls, loss_pnls], bins=10, 
        ...
        ax4.bar(['Avg Win', 'Avg Loss'], [avg_win, avg_loss],
               color=[self.colors['success'], self.colors['danger']], alpha=0.7)
    ...
    def plot_cash_position(
        ...
        # Plot stacked area
        ax.fill_between(equity_series.index, 0, cash_series.values,
                       label='Cash', color=self.colors['info'], alpha=0.6)
        ax.fill_between(equity_series.index, cash_series.values, equity_series.values,
                       label='Invested', color=self.colors['primary'], alpha=0.6)
        
        # Plot total equity line
        ax.plot(equity_series.index, equity_series.values,
               label='Total Equity', color='black', linewidth=2, linestyle='--')
