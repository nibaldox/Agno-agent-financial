...
class PortfolioMemoryManager:
    ...
    def __init__(self, initial_cash: float = 100.0, history_file: str | None = None):
        ...
        self.holdings = pd.DataFrame(columns=[
            'ticker', 'shares', 'buy_price', 'buy_date', 'stop_loss',
    ...
    def add_position(self, ticker: str, shares: float, buy_price: float, reason: str = "", stop_loss: Optional[float] = None):
        """
        ...
        if cost > self.cash:
            raise ValueError(f"Fondos insuficientes: ${cost:.2f} requerido, ${self.cash:.2f} disponible")
    ...
    def _record_trade(self, action: str, ticker: str, shares: float, price: float, amount: float, reason: str):
    ...
    def save_daily_snapshot(self):
        ...
        snapshot.to_csv(self.history_file, mode='a', header=not self.history_file.exists(), index=False)
    ...
    def get_historical_performance(self) -> Dict:
        ...
        return {
            ...
            } if not daily_df.empty else None,
            'worst_day': {
    ...
    def check_stop_losses(self) -> list:
        ...
        for idx, position in self.holdings.iterrows():
            ...
            if current_price > 0 and current_price != buy_price:
                ...
                open_price = float(df['Open'].iloc[-1]) if 'Open' in df.columns else float(df['Close'].iloc[-1])
