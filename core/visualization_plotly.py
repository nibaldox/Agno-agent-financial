...
class InteractiveVisualizationGenerator:
    ...
    def plot_portfolio_composition(self, holdings_df: pd.DataFrame) -> str:
        ...
        if holdings_df.empty:
            ...
            fig = go.Figure(data=[
                go.Pie(
                    ...
                    pull=[0.05 if i == 0 else 0 for i in range(len(composition))]  # Pull largest slice
                )
            ])
        ...
        fig.update_layout(
            **self.common_layout,
            title=dict(
                text='<b>🥧 Portfolio Composition</b>',
                font=dict(size=20),
                x=0.5,
                xanchor='center'
            ),
    ...
    def plot_win_loss_analysis(self, trades_df: pd.DataFrame) -> str:
        ...
        if trades_df.empty or 'PnL' not in trades_df.columns:
            ...
            colors = [self.colors['success'] if x > 0 else self.colors['danger'] for x in pnl_by_ticker.values]
    ...
    def plot_performance_vs_benchmark(
        ...
        # Bottom: Alpha
        colors_alpha = [self.colors['success'] if x > 0 else self.colors['danger'] for x in alpha.values]
        ...
        fig.update_layout(
            **self.common_layout,
            title=dict(
                text='<b>📊 Performance vs S&P 500</b>',
                font=dict(size=20),
                x=0.5,
                xanchor='center'
            ),
    ...
    def generate_all_plots(
        ...
        # 1. Daily performance
        charts['daily_performance'] = self.plot_daily_performance(
