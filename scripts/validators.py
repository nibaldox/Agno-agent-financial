...
class MicroCapValidator:
    ...
    def validate(self, ticker: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                valid=False, reason=f"Error al validar {ticker}: {str(e)}", severity="ERROR"
            )
...
class PositionSizingValidator:
    ...
    def validate_sector_exposure(
        ...
        try:
            ...
            return ValidationResult(
                valid=True, reason=f"⚠️ No se pudo validar sector: {str(e)}", severity="WARNING"
            )

    def validate_cash_reserve(
        self, cash_after_trade: float, total_equity: float
    ) -> ValidationResult:
        """
        ...
        return ValidationResult(
            valid=True, reason=f"✅ Cash reserve {cash_pct*100:.1f}% (suficiente)", severity="INFO"
        )
...
class TradeValidator:
    ...
    def validate_trade(
        ...
        if all_valid:
            results["overall"] = ValidationResult(
                valid=True, reason="✅ TODAS LAS VALIDACIONES PASARON", severity="INFO"
            )
        else:
            results["overall"] = ValidationResult(
                valid=False,
    ...
    def format_validation_report(self, results: Dict[str, ValidationResult]) -> str:
        ...
        for name, result in results.items():
            ...
            severity_icon = {"ERROR": "🔴", "WARNING": "⚠️", "INFO": "ℹ️"}.get(result.severity, "")
...
if __name__ == "__main__":
    ...
    results = validator.validate_trade(
        ticker="NVDA", position_value=15.0, cash=100.0, portfolio=pd.DataFrame(), total_equity=100.0
    )
    ...
    print("-" * 70)
    results = validator.validate_trade(
        ticker="ABEO", position_value=20.0, cash=100.0, portfolio=pd.DataFrame(), total_equity=100.0
    )
    print(validator.format_validation_report(results))
