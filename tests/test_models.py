...
Examples:
  ...
  python test_models.py --provider openrouter --model qwen/qwen-3-235b-instruct:free
    ...
    parser.add_argument(
        "--provider", type=str, choices=["deepseek", "openrouter"], help="LLM provider to test"
    )

    parser.add_argument(
        "--model", type=str, help="OpenRouter model ID (only used with --provider openrouter)"
    )

    parser.add_argument(
        "--list-models", action="store_true", help="List recommended OpenRouter models"
    )
