...
def test_2_stop_loss_monitoring():
    ...
    portfolio.add_position(ticker="TEST", shares=100, buy_price=50.0, stop_loss=45.0)
...
def test_3_live_trade_execution():
    ...
    result = executor.execute_buy(
        ticker="AAPL", shares=10, stop_loss=150.0, dry_run=True  # Modo dry-run para testing
    )
    ...
    result = executor.execute_sell(
        ticker="AAPL", shares=5, reason="Testing sell execution", dry_run=True
    )
...
def test_4_integrated_workflow():
    ...
    buy_result = executor.execute_buy(ticker="MSFT", shares=5, stop_loss=350.0, dry_run=True)

    if buy_result["success"]:
        # Añadir al portafolio
        portfolio.add_position(
            ticker="MSFT", shares=5, buy_price=buy_result["price"], stop_loss=350.0
        )
