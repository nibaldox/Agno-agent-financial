...
class LLMConfig:
    ...
    def call_llm(
        ...
        if use_tools and not self.supports_tools(model):
            print(
                f"⚠️ Warning: {model} doesn't support tools. Switching to {self.get_tool_model(provider)}"
            )
            model = self.get_tool_model(provider)
        ...
        if provider == "openrouter" and self.openrouter_client:
            try:
                # Use free model
                free_model = self.OPENROUTER_FREE_MODELS.get(
    ...
    def get_available_models(self) -> Dict[str, Any]:
        ...
        available = {
            ...
            "openrouter": {
                ...
                "models": (
                    list(self.OPENROUTER_FREE_MODELS.keys()) if self.openrouter_client else []
                ),
            },
        }
        return available
...
def get_llm_response(
    ...
    **kwargs,
    ...
    return llm_config.call_llm(
        prompt=prompt, system_message=system_message, model=model, provider=provider, **kwargs
    )
